...
{
    public class Skill1031Projectile : MonoBehaviour
    {
        [SerializeField]
        [Min(1)]
        private float speed;
        private Skill1031 skill;
        ...
        public void Init(Skill1031 skill) => this.skill = skill;
        ...
        void OnTriggerEnter2D(Collider2D collision)
        {
            if ((skill.Owner.DamageableLayerMask & (1 << collision.gameObject.layer)) != 0)
            {
                if (collision.gameObject.TryGetComponent<Monster>(out var monster))
                {
                    ...
                    monster.Slow(skill.SlowAmount / 100, skill.SlowDuration);
                }
            }
        }
    }
